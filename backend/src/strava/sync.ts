...
export async function syncStravaActivity(stravaId: number): Promise<void> {
  ...
  if (!resp.ok)
    throw new Error(`Strava activity fetch failed: ${resp.statusText}`);
  ...
  await deleteManualActivitiesForDateAndType(input.date, input.type).catch(
    (err) =>
      console.error(
        "Failed to delete planned activity after Strava sync:",
        err,
      ),
  );
}
...
export async function syncRecentStravaActivities(days = 2): Promise<number> {
  ...
  if (!resp.ok)
    throw new Error(`Strava activities list failed: ${resp.statusText}`);

  const activities = (await resp.json()) as StravaActivity[];
  for (const activity of activities) {
    const isRun = normaliseType(activity.sport_type ?? activity.type) === "run";
    if (isRun) {
      // Fetch full detail for runs so best_efforts (used for 5K time tracking) is preserved
      await syncStravaActivity(activity.id);
      continue;
    }
    const input: CreateActivityInput = {
