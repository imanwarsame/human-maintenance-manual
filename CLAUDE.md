...
This is a **monorepo health-tracking app** for a single user (Iman). Claude acts as the intelligence layer — writing meal plans, workout plans, coaching notes, and tracking illness/injury incidents — while the app is the logging UI and data store.
...
- **Router**: React Router v7 in `App.tsx`. Auth gate → magic link login, then 6 protected routes: `/` (Home), `/hydration`, `/activity`, `/nutrition`, `/progress`, `/settings`.
...
- `scheduler/` — cron-based hydration and mobility reminder jobs (web push)

### Database (Supabase)

Migrations in `supabase/migrations/`, applied in order — later ones depend on earlier ones:
1. `001_enums.sql` — custom enum types
2. `002_tables.sql` — core tables: `hydration_logs`, `activities`, `meal_plans`, `meal_completions`, `meal_deviations`, `coaching_notes`, `plan_context`
3. `003_indexes.sql` — performance indexes
4. `004_rls.sql` — row-level security policies
5. `005_planned_activities.sql` / `005_remove_deviations.sql` — `is_planned` flag on `activities`; drops the unused `meal_deviations` table
6. `006_exercise_weights.sql` — `exercise_weights` table
7. `007_body_weight.sql` — `body_weight_logs` table
8. `008_push_subscriptions.sql` — `push_subscriptions` table
9. `009_body_composition.sql` — adds `body_fat_pct`/`muscle_mass_kg` to `body_weight_logs`
